...
def collate_fn(data):
    def merge(sequences):
        ...
        padded_seqs = np.zeros(len(sequences), max(lengths)).long()
...
class SQuadDatasetWithTag():
    def __init__(self, src_file, trg_file, max_length, word2idx, debug=False,batch_size=64):
        ...
        self.trgs = open(trg_file, "r",encoding='utf-8').readlines()
    ...
    def iter(self):
        src_seqs, ext_src_seqs, trg_seqs, ext_trg_seqs, tag_seqs = [], [], [], [],[]
        idxs = range(self.num_seqs)
        ...
        for index in idxs:
            ...
            trg_seq, ext_trg_seq = self.question2ids(
                trg_seq, self.word2idx, oov_lst)
            if len(src_seqs) == self.batch_size or index == idxs[-1] :
                src_seqs.append(src_seq)
                ext_src_seqs.append(ext_src_seq)
                trg_seqs.append(trg_seq)
                ext_trg_seqs.append(trg_seq)
                tag_seqs.append(tag_seq)
                yield src_seqs, ext_src_seqs, trg_seqs, ext_trg_seqs, tag_seqs
                src_seqs, ext_src_seqs, trg_seqs, ext_trg_seqs, tag_seqs = [], [], [], [],[]
...
def make_embedding(embedding_file, output_file, word2idx):
    ...
    embedding = np.zeros((len(word2idx), 256), dtype=np.float32)
    num_oov = 0
    for word, idx in word2idx.items():
        embedding[idx] = word2vec[word2id.get(word, 0)]
...
def word_tokenize(tokens):
    return [token.replace("''", '"').replace("``", '"') for token in jieba.lcut(tokens)]
...
def process_file(file_name):
    ...
    with open(file_name, "r",encoding='utf-8') as f:
        ...
        # articles = source["data"][:10]
    for para in tqdm(articles):
        # for para in article["paragraphs"]:
        context = para["text"].replace(
            "''", '" ').replace("``", '" ').replace("\\n", '').replace("\n", '').replace("\\t", '').replace("\t", '').lower()

        context_tokens = word_tokenize(context)
        spans = convert_idx(context, context_tokens)

        for qa in para["annotations"]:
            total += 1
            ques = qa["Q"].replace(
                "''", '" ').replace("``", '" ').replace("\\n", '').replace("\n", '').replace("\\t", '').replace("\t", '').lower()
            ...
            for token in context_tokens:
                counter[token] += len(para["annotations"])
...
def make_conll_format(examples, src_file, trg_file):
    ...
    trg_fw = open(trg_file, "w",encoding='utf-8')
